...
class GiftComputer {
    public:
        ...
        bool is_valid_repeatable_id(const std::string& id) {
            // An ID is invalid if it consists of a pattern that repeates at least two times, i.e. 11 or 123123123

            for (size_t pattern_length = 1; pattern_length <= id.length() / 2; pattern_length++) {
                if (id.length() % pattern_length != 0) continue; // The length must be a multiple of the pattern length to repeat fully
                const std::string pattern = id.substr(0, pattern_length);
                bool all_match = true;
                for (size_t i = pattern_length; i < id.length(); i += pattern_length) {
                    if (id.substr(i, pattern_length) != pattern) {
                        all_match = false;
                        break; 
                    }
                }
                if (all_match) {
                    return false; // If we found a repeating pattern, the ID is invalid
                }
            }
            return true; // If no repeating pattern was found, the ID is valid
        }
...
static std::string solve_day2_part2(const std::vector<std::string>& lines) {
    ...
    for (const auto &id_range : id_ranges) {
        auto ids = computer.split_range(id_range);
        for (long id : ids) {
            if (!computer.is_valid_repeatable_id(std::to_string(id))) {
                computer.invalid_ids.push_back(id);
            }
        }
    }
